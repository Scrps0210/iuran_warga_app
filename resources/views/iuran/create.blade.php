...
@section('title', 'Tambah Iuran - Sistem Iuran Warga')

@section('content')
    <!-- Page Header -->
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-white">Tambah Iuran</h2>
            <p class="text-sm text-dark-text-secondary mt-1">Tambahkan data iuran baru</p>
        </div>
    ...
    <div class="rounded-lg bg-dark-card border border-dark-border">
        ...
        <form action="{{ route('iuran.store') }}" method="POST" class="p-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Warga -->
                <div class="md:col-span-2">
                    <label for="warga_id" class="mb-2.5 block text-sm font-medium text-white">Pilih Warga <span
                            class="text-danger">*</span></label>
                    <select name="warga_id" id="warga_id"
                        class="w-full rounded-lg border border-dark-border bg-dark-bg py-3 px-4 text-dark-text focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 @error('warga_id') border-danger @enderror">
                ...
                <div>
                    <label for="jumlah" class="mb-2.5 block text-sm font-medium text-white">Jumlah (Rp) <span
                            class="text-danger">*</span></label>
                    <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', 50000) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3" placeholder="Keterangan tambahan (opsional)"
                        class="w-full rounded-lg border border-dark-border bg-dark-bg py-3 px-4 text-dark-text placeholder-dark-text-secondary focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 @error('keterangan') border-danger @enderror">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="mt-1.5 text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
