...
@section('title', 'Tambah Warga - Sistem Iuran Warga')

@section('content')
    <!-- Page Header -->
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-white">Tambah Warga</h2>
            <p class="text-sm text-dark-text-secondary mt-1">Tambahkan data warga baru ke sistem</p>
        </div>
    ...
    <div class="rounded-lg bg-dark-card border border-dark-border">
        ...
        <form action="{{ route('warga.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama') }}" placeholder="Masukkan nama lengkap"
                ...
                <div>
                    <label for="nik" class="mb-2.5 block text-sm font-medium text-white">NIK <span
                            class="text-danger">*</span></label>
                    <input type="text" name="nik" id="nik" value="{{ old('nik') }}" placeholder="Masukkan 16 digit NIK"
                        maxlength="16"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat" id="alamat" rows="3" placeholder="Masukkan alamat lengkap"
                        class="w-full rounded-lg border border-dark-border bg-dark-bg py-3 px-4 text-dark-text placeholder-dark-text-secondary focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 @error('alamat') border-danger @enderror">{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="mt-1.5 text-xs text-danger">{{ $message }}</p>
                    @enderror
                </div>
